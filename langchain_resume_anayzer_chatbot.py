...
try:
    ...
    print("PyQt5 is not installed in this environment. Install it with: pip install PyQt5")
    sys.exit(1)
...
class ChatBot(QWidget):
    ...
    def langchain_openai_response(self, prompt_template,question):
        if ChatOpenAI is None or StrOutputParser is None or PromptTemplate is None:
            self.responseBox.setText("LangChain or Chat model not available. Install `langchain` and `langchain-chat`/`langchain_community` packages to enable LLM responses.")
            return
        try:
            ...
            self.responseBox.setText(f"Failed to invoke LLM: {str(e)}")
             
    def get_openai_response(self,messages):
        try:
            if openai is None:
                return "OpenAI SDK not installed. Install `openai` to use this function."
            if 'client' not in globals() or globals().get('client') is None:
                return "OpenAI client not configured. Initialize `client = openai.OpenAI(api_key=...)` to use this function."
        ...
        try:
            if fitz is not None:
                ...
                try:
                    ...
                    return "Failed to read pdf resume: install PyMuPDF or pypdf to read PDFs"
